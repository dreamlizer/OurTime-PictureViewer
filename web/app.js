...
async function openFolder(path=''){const scope=state.folderTarget==='scan'?'scan':'browse';const data=await api('/api/folders?scope='+scope+'&path='+encodeURIComponent(path));state.folder=data.path;$('#folder-current').textContent=data.path||'本机磁盘';$('#folder-list').innerHTML=(data.parent!==null?`<button data-folder="${esc(data.parent)}">↑ 返回上一级</button>`:'')+data.items.map(x=>`<button data-folder="${esc(x.path)}">▱ ${esc(x.name)}</button>`).join('');$('#use-folder').disabled=!data.path;showDialog('#folder-dialog');}
