...
const state={view:'timeline',q:'',person:'',place:'',dateFrom:'',dateTo:'',offset:0,total:0,items:[],people:[],passersby:[],selected:new Set(),peopleSelected:new Set(),peopleMerging:false,selecting:false,detail:null,personId:null,status:null,folder:null,directory:'',sort:'date_desc',maxId:0,folderTarget:'scan',scanRoots:[],scanSubmitting:false,scanStartedThisVisit:false,scanShowCompletedResult:false,exclusion:null,thumbRevision:0,peopleStream:{people:{items:[],offset:0,total:0,more:true,loading:false,q:'',generation:0},passersby:{items:[],offset:0,total:0,more:true,loading:false,q:'',generation:0}}};
...
function updateChrome(){
  ...
  if(organizeNav) organizeNav.open=organize || organizeNav.dataset.pinned==='1';
  $$('.nav[data-view]').forEach(el=>{
    const inOrganize=organizeViews().has(el.dataset.view);
...
function setPeopleMerging(on){state.peopleMerging=Boolean(on);if(!state.peopleMerging)state.peopleSelected.clear();$$('#people-grid .person-card').forEach(card=>{if(!state.peopleMerging)card.classList.remove('selected');});$$('#people-grid [data-select-person]').forEach(box=>{if(!state.peopleMerging)box.checked=false;});updatePeopleMerge();}
