...
async function openPerson(id){state.personId=id;hideNotice();state.personFaces={generation:0,offset:0,loading:false};const p=await api('/api/people/'+id+'?limit=48');state.personDetail=p;state.personFaces.offset=(p.faces||[]).length;$('#person-eyebrow').textContent=p.ignored?'路人':'熟悉的面孔';$('#person-title').textContent=personLabel(p);const count=p.face_count||(p.faces||[]).length;$('#person-help').hidden=true;$('#person-title').dataset.count=fmt(count);$('#person-name').value=p.ignored?'':((p.name&&p.name!=='待核对')?p.name:'');$('#person-alias').value=p.alias||'';$('#ignore-person').textContent=p.ignored?'恢复到人物档案':'标为路人';await fillMergeTargets('', p.suggested_person_id);renderPersonFaces(true);showDialog('#person-dialog');bindPersonFaceScroll();maybeLoadMorePersonFaces();}
