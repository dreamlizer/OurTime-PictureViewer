...
const state={view:'timeline',q:'',person:'',offset:0,total:0,items:[],people:[],passersby:[],selected:new Set(),peopleSelected:new Set(),selecting:false,detail:null,personId:null,status:null,folder:null,directory:'',sort:'date_desc',maxId:0,folderTarget:'scan',scanRoots:[],scanSubmitting:false,exclusion:null,thumbRevision:0,peopleStream:{people:{items:[],offset:0,total:0,more:true,loading:false,q:'',generation:0},passersby:{items:[],offset:0,total:0,more:true,loading:false,q:'',generation:0}}};
const titles={all:['全部照片','全部照片'],folders:['文件夹','只看 I 盘。'],people:['人物档案','那些熟悉的面孔。'],passersby:['路人','先不识别，以后还能找回来。'],timeline:['全部照片','全部照片'],years:['按年份查看','点某一年，只看那一年。'],places:['按地点','记得那是在哪里。'],objects:['物体','照片里有什么。'],groups:['合影','两个人以上，按认出的人脸来分。'],uncertain:['待确认时间','给记忆一个时间。'],no_place:['待补充地点','记得那是在哪里吗？'],duplicates:['重复副本','一张照片，多个来处。'],screenshots:['截图与小图','日常的片段，也有位置。'],errors:['读取问题','把未完成的部分看清楚。'],missing:['原文件缺失','寻找照片现在的位置。'],scan:['添加照片','添加照片'],excluded:['已排除','留下值得保存的记忆。']};
...
function updateTimelineTools(){const on=isTimelineView();$('#timeline-tools').hidden=!on;if(on){$$('#timeline-tools [data-time-sort]').forEach(b=>b.classList.toggle('active',b.dataset.timeSort===state.sort));$('#timeline-by-year').classList.toggle('active',state.view==='years');}document.body.classList.toggle('is-home-view',on&&state.view==='timeline');document.body.classList.toggle('is-add-photos-view',state.view==='scan');window.__ourTimeHomeSync?.();}
...
function personCard(p,mode='people'){
  ...
  const relation=!named&&Number(p.known_connection_count)>0?`<small class="person-connection">与 ${esc(p.best_known_person_name||'已命名人物')} 同框 ${fmt(p.best_known_shared_count||0)} 张${Number(p.known_connection_count)>1?' · 关联 '+fmt(p.known_connection_count)+' 位已命名人物':''}</small>`:'';
  return `<article class="person-card ${state.peopleSelected.has(p.id)?'selected':''}" data-person="${p.id}"><label class="person-check"><input type="checkbox" data-select-person="${p.id}" ${state.peopleSelected.has(p.id)?'checked':''} aria-label="选择这个人"></label><button type="button" class="person-open" data-open-person="${p.id}"><img src="/api/face/${p.cover}" alt="人物候选缩略图" loading="lazy"><b>${esc(personLabel(p))}</b><p>${p.photo_count} 张</p>${relation}</button>${named?'':'<button type="button" class="text-button person-quick-name" data-quick-name="'+p.id+'">命名</button><button type="button" class="text-button person-ignore" data-ignore-person="'+p.id+'">标为路人</button>'}</article>`;
}
...
$('#start-scan').addEventListener('click',action(async()=>{const roots=[...(state.scanRoots||[])];if(!roots.length)throw new Error('请先选择照片文件夹');if(state.scanSubmitting)return;state.scanSubmitting=true;renderScanRoots();try{await api('/api/scan',{method:'POST',body:JSON.stringify({roots,with_faces:true,include_system:false,workers:1})});state.scanRoots=[];toast('开始扫描，已入库的照片会自动跳过');await refreshStatus();}finally{state.scanSubmitting=false;renderScanRoots();}}));$('#scan-view-all').addEventListener('click',action(()=>setView('timeline')));
...
window.__ourTimeApp={state,api,peopleQuery,personLabel,showDialog,openFolder,exclusionDialog,restoreAssets};
