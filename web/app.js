...
const titles={home:['首页','从往年里看见今天。'],'home-group':['首页','推荐中的照片'],all:['全部照片','全部照片'],folders:['文件夹','浏览已添加的文件夹。'],people:['人物档案','人物档案'],passersby:['路人','不在主要人物列表里展示。'],timeline:['全部照片','全部照片'],'public-figures':['公众人物','公众人物'],years:['按年份查看','点某一年，只看那一年。'],places:['按地点','记得那是在哪里。'],objects:['物体','照片里有什么。'],groups:['合影','合影'],favorites:['收藏','收藏的照片'],uncertain:['待确认时间','给记忆一个时间。'],no_place:['待补充地点','记得那是在哪里吗？'],duplicates:['重复副本','一张照片，多个来处。'],screenshots:['截图与小图','日常的片段，也有位置。'],errors:['读取问题','把未完成的部分看清楚。'],missing:['原文件缺失','寻找照片现在的位置。'],scan:['添加照片','添加照片'],excluded:['已排除','留下值得保存的记忆。']};
...
function preparePanel(panel){
  ...
  panel._pendingTimer=setTimeout(()=>{if(panel.classList.contains('is-switching'))panel.classList.add('is-pending');},220);
}
function revealPanel(panel, token){
  ...
  requestAnimationFrame(()=>{
    if(token&&!isCurrentView(token))return;
    panel.classList.remove('is-switching');
    panel.classList.add('is-ready');
    panel.dataset.motionOwner='';
  });
}
function planViewTransition(fromView,toView,{samePhotoStream=false}={}){
  // One content-opacity owner per switch. Same-stream swaps never fade the panel.
  if(samePhotoStream)return {owner:'grid',kind:'source-swap',sameStream:true};
  if(fromView===toView)return {owner:'none',kind:'noop',sameStream:false};
  return {owner:'panel',kind:'panel-fade',sameStream:false};
}
...
function resetPhotoStream(){waterfall.abort?.abort();waterfall.abort=new AbortController();waterfall.generation++;waterfall.pending.clear();waterfall.cache.clear();waterfall.heights=[];waterfall.total=0;waterfall.maxId=0;waterfall.error=false;waterfall.query=null;streamEntrance.disconnect();const grid=$('#photo-grid');const keep=grid?grid.getBoundingClientRect().height:0;if(grid){grid.replaceChildren();if(keep>80){grid.style.minHeight=keep+'px';grid.style.height='';}else{grid.style.height='0px';grid.style.minHeight='';}}$('#stream-status').textContent='正在加载照片…';$('#stream-retry').hidden=true;$('#no-results').hidden=true;$('#empty').hidden=true;$('#result-count').textContent='加载中';syncGroupResultCount(state.view,{clear:true});}
function currentPlaceName(){return String(state.view||'').startsWith('place:')?state.view.slice(6):'';}
function capturePlaceMapState(anchor=null){
  const map=state.placeMap;if(!map)return null;
...
async function returnToPlacesMap(){
  const saved=state.placeReturn&&{...state.placeReturn,center:[...state.placeReturn.center]};
  if(saved&&state.placeMap){
    clearTimeout(state.placeMapTimer);
    state.placeMap.setView(saved.center,saved.zoom,{animate:false});
  }
  await setView('places');
  if(saved&&state.placeMap){
    await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
    state.placeMap.invalidateSize();
    state.placeMap.setView(saved.center,saved.zoom,{animate:false});
    updatePlaceZoomInput();
    await loadPlaceMap();
  }
  state.placeReturn=null;
}
function placeAnchorDistance(photo,hint){
  if(!hint)return 0;
  const latitude=Number(photo.latitude),longitude=Number(photo.longitude);
  const north=(latitude-Number(hint.latitude))*111132;
  const east=(longitude-Number(hint.longitude))*111320*Math.max(.01,Math.abs(Math.cos(Number(hint.latitude)*Math.PI/180)));
  return north*north+east*east;
}
async function findPlaceDetailAnchor(place){
  const hint=state.placeReturn&&state.placeReturn.anchor;
  if(hint&&hint.id){
    const photo=await api('/api/photos/'+Number(hint.id));
    if(hasPhotoCoordinates(photo))return photo;
  }
  let offset=0,total=1,best=null,bestDistance=Infinity;
  while(offset<total){
    const params=new URLSearchParams({filter:'place:'+place,offset:String(offset),limit:'120',sort:'date_desc'});
    const data=await api('/api/photos?'+params);
    total=Number(data.total)||0;
    for(const photo of data.items||[]){
      if(!hasPhotoCoordinates(photo))continue;
      const distance=placeAnchorDistance(photo,hint);
      if(distance<bestDistance){best=photo;bestDistance=distance;}
    }
    if(best)break;
...
async function openPlaceDetailEditor(){
  const place=currentPlaceName();
  if(!place||place==='unknown')throw new Error('这个地点没有可修改的 GPS 坐标');
  const photo=await findPlaceDetailAnchor(place);
  if(!photo)throw new Error('这个地点没有可用于范围修改的 GPS 坐标');
  await showPhotoPlaceMap(photo,{returnView:state.view,returnMode:'view',context:currentBrowseContext()});
  await openPhotoPlaceEditor();
}
...
function updateChrome(){
  ...
  const showStats=view==='timeline'||view==='public-figures';
  document.body.classList.toggle('is-home-discovery',view==='home');
  document.body.classList.toggle('is-home-group',view==='home-group');
  ...
  document.body.classList.toggle('is-place-detail-view',String(view).startsWith('place:'));document.body.classList.toggle('is-public-figures',view==='public-figures');const publicMastheadBack=$('#public-figures-masthead-back');if(publicMastheadBack)publicMastheadBack.hidden=view!=='public-figures';
  const placeDetail=String(view).startsWith('place:');
  const placeBack=$('#place-detail-back');if(placeBack)placeBack.hidden=!placeDetail;
  const placeEdit=$('#place-detail-edit');if(placeEdit)placeEdit.hidden=!placeDetail||currentPlaceName()==='unknown';
  const stickyBrowserView=view==='people'||['home','home-group','timeline','public-figures','folders','places','groups','favorites'].includes(view)||String(view).startsWith('place:')||String(view).startsWith('group:')||isPhotoPlaceView(view);
  ...
  $$('.nav[data-view]').forEach(el=>{
    el.classList.toggle('active',el.dataset.view===view||(el.dataset.view==='home'&&(view==='home'||view==='home-group'))||(el.dataset.view==='timeline'&&(isTimelineView(view)||view==='public-figures'))||(el.dataset.view==='places'&&(String(view).startsWith('place:')||isPhotoPlaceView(view)))||(el.dataset.view==='groups'&&isGroupView(view)));
  });
}
async function loadFolderBrowser(path, viewToken=null){
  ...
  try{
    ...
    if(trail){
      trail.hidden=false;
      const crumbs=[];
      crumbs.push('<button type="button" class="trail-item'+(depth?'':' current')+'" data-folder-open="">本机</button>');
      if(parts.length){
    ...
    list.innerHTML=items.map(item=>{
      ...
      const label=depth===0?(folderDrive(item.path)||item.name):item.name;
      const add=depth===0?'<button type="button" class="text-button folder-add" data-folder-add="'+esc(item.path)+'">添加这里的照片</button>':'';
    ...
    }).join('')||'<div class="no-results">这里还没有可看的文件夹。</div>';
...
function syncScanActivity(){const progress=$('#scan-progress');if(!progress)return;const job=state.status?.job;const serverActive=Boolean(job&&['running','pausing'].includes(job.status));const fresh=Boolean(state.scanStatusAt&&Date.now()-state.scanStatusAt<15000);const live=serverActive&&fresh;progress.classList.toggle('is-live',live);progress.classList.toggle('is-stale',serverActive&&!fresh);const heartbeat=$('#scan-heartbeat');if(heartbeat)heartbeat.title=live?'后台正在运行':serverActive?'暂未收到后台状态':'扫描未运行';}
function renderScanStatus(data){
  state.scanStatusAt=Date.now();
...
function currentPageTitle(view, fallback){
  ...
  if(view==='home-group')return state.homeGroupTitle||'推荐中的照片';
...
function syncGroupResultCount(view=state.view, {clear=false, total=null}={}){
  ...
  if(!el)return;
  const on=String(view||'').startsWith('group:')||view==='public-figures';
...
async function setView(view){window.OurTimeContinuity?.leaving();if(view==='all')view='timeline';if(view!=='home-group'&&view!=='home'){state.homeSnapshotId='';} if(!String(view).startsWith('place:'))state.placeMapFilter=null;const previousView=state.view;if(previousView==='scan'&&view!=='scan'){state.scanStartedThisVisit=false;state.scanShowCompletedResult=false;}if(view==='scan'&&previousView!=='scan'){state.scanStartedThisVisit=false;state.scanShowCompletedResult=false;}const homeStream=v=>v==='timeline'||v==='public-figures'||String(v||'').startsWith('group:');const shelfSwitch=(previousView==='timeline'||previousView==='public-figures')&&(view==='timeline'||view==='public-figures')&&previousView!==view;if(shelfSwitch){state.savedPhotoQuery=state.savedPhotoQuery||{};state.savedPhotoQuery[previousView]={q:state.q,person:state.person,directory:state.directory,place:state.place,dateFrom:state.dateFrom,dateTo:state.dateTo,publicGroup:state.publicGroup||''};const saved=state.savedPhotoQuery[view]||{q:'',person:'',directory:'',place:'',dateFrom:'',dateTo:'',publicGroup:''};state.q=saved.q||'';state.person=saved.person||'';state.directory=saved.directory||'';state.place=saved.place||'';state.dateFrom=saved.dateFrom||'';state.dateTo=saved.dateTo||'';state.publicGroup=saved.publicGroup||'';}const leavingHome=homeStream(previousView);const enteringHome=homeStream(view);if(leavingHome&&!enteringHome){state.q='';state.person='';state.directory='';state.place='';state.dateFrom='';state.dateTo='';}if(['home','people','passersby','scan','places','years','groups','objects'].includes(view)||String(view).startsWith('year:')||String(view).startsWith('place:')||String(view).startsWith('group:')||view==='home-group')resetPhotoStream();window.OurTimeContinuity?.applyPending(view);state.view=view;if(view==='home'){document.body.classList.add('is-home-discovery');const hv=$('#home-view'),lv=$('#library-view');if(hv)hv.hidden=false;if(lv)lv.hidden=true;}else{document.body.classList.remove('is-home-discovery');}const viewToken=beginViewChange(view);$('#exclusion-panel').hidden=view!=='excluded';if(view==='excluded')await loadExclusionRules(viewToken);if(!isCurrentView(viewToken, view))return;state.offset=0;state.selected.clear();state.selecting=false;if(view!=='people')setPeopleMerging(false);updateBatch();$$('.nav[data-view]').forEach(el=>el.classList.toggle('active',el.dataset.view===view||(el.dataset.view==='home'&&(view==='home'||view==='home-group'))||(el.dataset.view==='timeline'&&(isTimelineView(view)||view==='public-figures'))||(el.dataset.view==='places'&&String(view).startsWith('place:'))||(el.dataset.view==='groups'&&isGroupView(view))));const title=titles[view]||(isTimelineView(view)?timelineTitle(view):isGroupView(view)?groupTitle(view):String(view).startsWith('place:')?['按地点',view.slice(6)]:[view,view]);const isGroupDetail=String(view).startsWith('group:');$('#breadcrumb').textContent=title[0];$('#page-title').textContent=currentPageTitle(view,title[1]);const groupsBack=$('#groups-back');if(groupsBack)groupsBack.hidden=!isGroupDetail;syncGroupResultCount(view,{clear:true});const samePhotoStream=(previousView==='timeline'||previousView==='public-figures')&&(view==='timeline'||view==='public-figures')&&previousView!==view;const panel=viewPanel(view);const motion=planViewTransition(previousView,view,{samePhotoStream});if(panel)panel.dataset.motionOwner=motion.owner;if(samePhotoStream&&panel){panel.classList.remove('is-switching','is-pending');panel.classList.add('is-ready');}else if(motion.owner==='panel')preparePanel(panel);else if(panel)panel.classList.remove('is-switching','is-pending');const hideLibrary=['home','people','passersby','scan','years','places','groups','folders'].includes(view) && !isGroupDetail;$('#library-view').hidden=hideLibrary || view==='home';const homeView=$('#home-view');if(homeView)homeView.hidden=view!=='home';const homeBack=$('#home-group-back');if(homeBack)homeBack.hidden=view!=='home-group';$('#people-view').hidden=view!=='people';$('#passersby-view').hidden=view!=='passersby';$('#timeline-view').hidden=view!=='years';$('#places-view').hidden=view!=='places';$('#groups-view').hidden=view!=='groups';$('#objects-view').hidden=view!=='objects';$('#scan-view').hidden=view!=='scan';const foldersView=$('#folders-view');if(foldersView)foldersView.hidden=view!=='folders';$('#collection-title').textContent=title[0];const publicBack=$('#public-figures-back');if(publicBack)publicBack.hidden=true;updateChrome();updateTimelineTools();try{if(view==='people'){state.peopleSelected.clear();setPeopleMerging(false);await loadPeople(viewToken);}else if(view==='passersby')await loadPassersby(viewToken);else if(view==='years')await loadTimeline(viewToken);else if(view==='places')await loadPlaces(true,viewToken);else if(view==='groups')await loadGroups(viewToken);else if(view==='objects'){await setView('timeline');return;}else if(view==='folders')await loadFolderBrowser(state.folderPath||'',viewToken);else if(view==='scan'){renderScanRoots();await refreshStatus();}else if(view==='home'){if(typeof loadHomeDiscovery==='function')await loadHomeDiscovery({category:state.homeCategory||'all',signal:viewToken.signal});else await refreshStatus();}else {if((isTimelineView(view)||String(view).startsWith('group:')||view==='public-figures')&&!['date_asc','date_desc'].includes(state.sort)){state.sort='date_desc';$('#sort-order').value=state.sort;}await loadPhotos(samePhotoStream?{preserveUntilReady:true,scrollTop:true}:{});refreshStatus();}if(isCurrentView(viewToken,view)){revealPanel(panel,viewToken);await window.OurTimeContinuity?.ready();}}catch(err){if(!(err&&err.name==='AbortError'))throw err;}}

async function refreshStatus(){const previous=state.status;const data=await api('/api/status',{timeoutMs:15000});const previousActive=Boolean(previous?.job&&['running','pausing','paused'].includes(previous.job.status));const terminal=Boolean(data.job&&['completed','completed_with_errors','failed'].includes(data.job.status));if(state.view==='scan'&&state.scanStartedThisVisit&&previousActive&&terminal)state.scanShowCompletedResult=true;state.status=data;const s=data.stats;setText('#s-assets',fmt(s.assets));setText('#nav-count',fmt(s.assets));setText('#excluded-count',fmt(s.excluded_assets));if(typeof s.group_photos==='number'&&Number.isFinite(s.group_photos))setText('#groups-count',fmt(s.group_photos));state.connectionNotice=false;setText('#s-people',fmt(s.people));setText('#people-count',fmt(s.people));setText('#s-uncertain',fmt(s.uncertain_dates));setText('#s-duplicates',fmt(s.duplicates));const j=data.job;renderScanStatus(data);const active=j&&['running','pausing'].includes(j.status);const bannerProgress=j?.phase==='inventory'?`正在清点 · 已发现 ${fmt(j.discovered)} 张照片`:j?.total?`已检查 ${fmt(j.processed)} / ${fmt(j.total)}`:`本轮已检查 ${fmt(j?.processed)} 个文件`;const running=$('#running-dot'); if(running) running.className=active?'active':'';setDisabled('#start-scan',Boolean(active)||!state.scanRoots.length||state.scanSubmitting);setHidden('#job-banner',!active);setHtml('#job-banner',active?`<span>● ${statuses[j.status]} · ${bannerProgress}</span><button class="text-button" id="banner-details">查看进度 →</button>`:'');setText('#job-status',j?statuses[j.status]:'');if(j)setHtml('#job-details',`<div>${esc(JSON.parse(j.roots).join('；'))}</div><div class="job-metrics"><span><b>${fmt(j.discovered)}</b>本轮发现文件</span><span><b>${fmt(j.processed)}</b>本轮已检查</span><span><b>${fmt(j.metadata_reads)}</b>新读取元数据</span><span><b>${fmt(j.skipped)}</b>已完成直接跳过</span><span><b>${fmt(j.errors)}</b>读取问题</span></div><p>${esc(j.message||'正在扫描；已完成文件会快速跳过。')} · ${j.workers} 路处理 · 近一分钟新资料读取 ${data.metadata_per_second} 张/秒 · 略过 ${fmt(j.auxiliary)} 个辅助文件</p><div class="path-line">${esc(j.current_path)}</div>`);setHidden('#error-details',!data.errors.length);setText('#error-summary',`读取问题：共 ${fmt(j?.errors)} 项，显示最近 ${data.errors.length} 项`);setHtml('#scan-errors',data.errors.map(e=>`<div class="error-row"><b>${esc(e.stage)}</b><div>${esc(e.path)}</div><div>${esc(readableError(e.message))}</div><details><summary>原始诊断（技术信息）</summary><pre>${esc(e.message)}</pre></details></div>`).join(''));const cap=data.capabilities;const edition=document.getElementById('edition-label');if(edition) edition.textContent=(cap.product_name||'拾光相册')+(cap.version?(' · '+cap.version):'');const setup=document.getElementById('setup-status');if(setup){ const bits=[];if(!cap.face_model) bits.push('未找到人脸模型，添加照片时不会自动识别');if(!cap.geo) bits.push('未找到地名数据，将保留 GPS 和人工地点');if(!cap.exiftool) bits.push('未找到 ExifTool，只能读取基础拍摄信息');if(cap.export_renderer===false) bits.push('带人名标签导出需要可选的 Chromium 组件');if(cap.face_labels===false) bits.push('未配置人名底牌，部分主题会退回基础样式');if(cap.idle_exit_seconds) bits.push('关闭网页后约 '+cap.idle_exit_seconds+' 秒会退出后台，扫描进行中除外');setup.textContent=bits.length?(bits.join('。')+'。'):'本机组件已就绪。'; }setHtml('#capabilities',`人脸模型：${cap.face_model?'已找到 · '+(cap.face_runtime||'处理器')+' 运行':'未找到'}<br>HEIC / HEIF：${cap.heif?'可读取':'尚未安装解码组件，遇到时会记录问题'}<br>扩展元数据：${cap.exiftool?'已就绪 · 常驻进程':'仅基础读取器'}<br>离线地名：${cap.geo?'中文及海外地名数据已就绪':'未找到，保留 GPS 原始坐标'}<br>资料库：${esc(cap.data_dir)}`);if(previousActive&&!active&&terminal){toast(j.message||'扫描结束');if(state.view==='people')await loadPeople();else if(state.view==='passersby')await loadPassersby();else if(state.view==='home'){if(typeof loadHomeDiscovery==='function')await loadHomeDiscovery({category:state.homeCategory||'all'});}else if(!['scan'].includes(state.view))await loadPhotos();}if(j&&!state.scanPrefilled){$('#scan-roots').value=JSON.parse(j.roots).join('\n');$('#scan-workers').value=String(j.workers);state.scanPrefilled=true;}return data;}
function personLabel(p){if(p.ignored)return p.name||'路人';return p.alias?(p.name||('待命名 '+p.id))+' / '+p.alias:(p.name||('待命名 '+p.id));}
function personStatus(p){if(p.ignored)return '路人';if(p.confirmed)return p.alias?'已命名 · '+p.alias:'已人工确认';return p.suggested_name?'可能是 '+p.suggested_name:'待核对分组';}
...
function syncPlaceModeChrome(photo=null){
  ...
  if(back){back.hidden=!single;back.textContent=state.photoMap&&state.photoMap.returnMode==='view'?'返回地点照片':'返回大图';}
...
async function savePhotoPlaceRange(){
  ...
  try{
    const result=await editPhotoNearby(Number(saved.id),{place,radius_m:radius});
    saved.photo={...saved.photo,manual_place:result.name,effective_place:result.name};saved.nearby=null;saved.editorOpen=false;
    if(saved.returnMode==='view')saved.returnView='place:'+result.name;
...
async function showPhotoPlaceMap(photo,options={}){
  ...
  updateChrome();updateTimelineTools();const panel=viewPanel(state.view);preparePanel(panel);
...
async function reopenPhotoFromPlaceMap(){
  ...
  if(saved.returnMode==='view')return;
...
async function openPerson(id){
 ...
 state.personDetail=p;state.personFaces.offset=(p.faces||[]).length;$('#person-eyebrow').textContent=p.ignored?'路人':'熟悉的面孔';$('#person-title').textContent=personLabel(p);const count=p.face_count||(p.faces||[]).length;const named=Boolean(!p.ignored&&isNamedPerson(p));$('#person-dialog').dataset.personMode=named?'named':(p.ignored?'ignored':'pending');$('#person-help').hidden=true;$('#person-title').dataset.count=fmt(count);$('#person-face-count').textContent=fmt(count)+' 张';const organize=$('#person-organize');if(organize){organize.open=!named;organize.querySelector('summary').textContent=p.ignored?'整理这位路人':'整理此人物';}const photosButton=$('#show-person-photos');if(photosButton){photosButton.textContent='查看相关照片 · '+fmt(count)+' 张';photosButton.className=(named?'primary':'secondary')+' person-photos-primary';}$('#person-name').value=p.ignored?'':((p.name&&p.name!=='待核对')?p.name:'');$('#person-alias').value=p.alias||'';draftRevision.person=0;if(!p.ignored&&p.confirmed&&p.name&&p.name!=='待核对')setPersonSaveState('已标记为：'+p.name);$('#ignore-person').textContent=p.ignored?'恢复到人物档案':'标为路人';
 ...
 renderPersonFaces(true);const personDialog=$('#person-dialog');const personWasOpen=Boolean(personDialog&&personDialog.open);showDialog('#person-dialog');if(personWasOpen&&personDialog){personDialog.classList.remove('person-swap');void personDialog.offsetWidth;personDialog.classList.add('person-swap');}bindPersonFaceScroll();maybeLoadMorePersonFaces();return true;
}
...
$('#folders-up')&&$('#folders-up').addEventListener('click',action(async()=>{const slash=String.fromCharCode(92); const parent=(state.folderNav&&state.folderNav.parent)||''; await loadFolderBrowser(parent);}));
...
$$('[data-view]').forEach(b=>b.addEventListener('click',action(()=>window.OurTimeContinuity?window.OurTimeContinuity.navigate(b.dataset.view):setView(b.dataset.view))));$$('[data-jump]').forEach(b=>b.addEventListener('click',action(()=>setView(b.dataset.jump))));['#add-folder','#empty-add'].forEach(s=>$(s).addEventListener('click',action(()=>openAddPhotos())));$$('[data-close]').forEach(b=>b.addEventListener('click',()=>{const top=$$('dialog[open]').at(-1);if(top&&top.id!==b.dataset.close)return;$('#'+b.dataset.close).close();}));
...
$('#person-dialog')&&$('#person-dialog').addEventListener('close',()=>{const dialog=$('#person-dialog');if(dialog){delete dialog.dataset.photoReturn;delete dialog.dataset.photoReturnScroll;dialog.inert=false;}entitySessions.close('person');});
$('#person-dialog')&&$('#person-dialog').addEventListener('cancel',e=>{if($('#detail-dialog')?.open){e.preventDefault();closePhotoViewer();}});
...
$('#public-figures-back')?.addEventListener('click',action(()=>setView('timeline')));$('#public-figures-masthead-back')?.addEventListener('click',action(()=>setView('timeline')));
...
$('#batch-form').addEventListener('submit',action(async e=>{e.preventDefault();const body={ids:[...state.selected]};if($('#batch-date-enabled').checked){body.manual_date=$('#batch-date').value;body.manual_precision=$('#batch-precision').value;}if($('#batch-place-enabled').checked)body.manual_place=$('#batch-place').value;if($('#batch-notes-enabled').checked)body.notes=$('#batch-notes').value;const receipt=await editPhotoMetadata(body);$('#batch-dialog').close();toast(photoEditMessage(receipt));await refreshStatus();await loadPhotos();}));

$('#detail-form').addEventListener('submit',action(async e=>{e.preventDefault();const id=Number(state.detail&&state.detail.id),payload={ids:[id],manual_date:$('#edit-date').value,manual_precision:$('#edit-precision').value,manual_place:$('#edit-place').value,notes:$('#edit-notes').value},revision=draftRevision.photo.get(id)||0,write=captureEntityWrite('photo',id,payload,revision);const receipt=await queueEntityWrite('photo',id,()=>editPhotoMetadata(write.payload,write.operationId));const submittedRevisionIsLatest=(draftRevision.photo.get(id)||0)===write.draftRevision;if(submittedRevisionIsLatest)viewer.drafts.delete(id);if(submittedRevisionIsLatest&&entitySessions.current(write.entitySession)&&Number(state.detail&&state.detail.id)===id){toast(photoEditMessage(receipt));await openPhoto(id);await refreshStatus();await loadPhotos();}}));
...
document.addEventListener('click',action(async e=>{
 ...
 if(photo){
  if(photo.dataset.photo==='shelf'){if(!state.selecting)await setView('public-figures');return;}
  if(photo.dataset.publicRel && !Number(photo.dataset.photo)){const img=$('#detail-img');if(img){img.alt='公众人物';img.src='/api/public-figures/media?rel='+encodeURIComponent(photo.dataset.publicRel);const dialog=$('#detail-dialog');if(dialog&&!dialog.open)dialog.showModal();}return;}
 ...
 if(facePhoto){
  ...
  if(personDialog&&body)personDialog.dataset.photoReturnScroll=String(body.scrollTop||0);
  if(personDialog){personDialog.dataset.photoReturn='1';personDialog.inert=true;}
  try{
   ...
   if(!opened)showNotice('这张照片已不在当前有效范围，人物窗口仍为你保留。',true);
  }finally{
   if(personDialog&&!$('#detail-dialog')?.open){personDialog.inert=false;delete personDialog.dataset.photoReturn;delete personDialog.dataset.photoReturnScroll;}
  }
 }
...
window.__ourTimeApp={state,api,operationRequest,peopleQuery,personLabel,prettyPlace,showDialog,openFolder,exclusionDialog,restoreAssets,refreshStatus,setView,openAddPhotos,addScanRoot,renderScanRoots,syncScanActivity,captureEntityWrite,queueEntityWrite,entitySessions,entitySession,draftRevision,preparePhotoDetail};
...
setInterval(()=>{fetch('/api/ui-ping',{method:'POST',cache:'no-store'}).catch(()=>{});},3000);
fetch('/api/ui-ping',{method:'POST',cache:'no-store'}).catch(()=>{});

const scanCard=document.querySelector('.add-photos-card');
if(scanCard){
  scanCard.addEventListener('dragover',e=>{e.preventDefault();scanCard.classList.add('is-drop');});
  scanCard.addEventListener('dragleave',()=>scanCard.classList.remove('is-drop'));
  scanCard.addEventListener('drop',e=>{
    e.preventDefault();
    scanCard.classList.remove('is-drop');
    toast('浏览器不能读取拖入文件夹的完整路径。请点“选择照片文件夹”，或在启动窗口里拖入。',true);
    state.folderTarget='scan';
    openFolder();
  });
}
...
function shouldShowPublicShelf(){
 const shelf=state.publicFigures;
 return state.view==='timeline'&&!state.q&&!state.person&&!state.directory&&!state.place&&!state.dateFrom&&!state.dateTo&&shelf&&shelf.enabled&&(!shelf.ready||Number(shelf.count)>0);
}
let publicFigureTimer=0;
async function syncPublicFigures(reload){
 clearTimeout(publicFigureTimer);
 let data;
 try{data=await api('/api/public-figures');}
 catch(e){data={enabled:false,ready:true,scanning:false,count:0,revision:0};}
 const previous=state.publicFigures;
 state.publicFigures=data;
 const changed=!previous||previous.revision!==data.revision||previous.ready!==data.ready||previous.count!==data.count;
 if(reload&&changed&&data.ready&&(state.view==='timeline'||state.view==='years'||state.view==='public-figures')){
  if(state.view==='years')await loadTimeline();
  else await loadPhotos();
 }
 if(data.enabled&&(!data.ready||data.scanning))publicFigureTimer=setTimeout(()=>syncPublicFigures(true),1200);
}
