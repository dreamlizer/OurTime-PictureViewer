...
浏览器的全部照片、文件夹、地点、合影和人物档案视图统一固定顶栏、标题和工具区；390px 等窄屏不得横向溢出。大图“素笺”主题使用 `web/vendor/fonts/` 内置屏幕阅读版文楷；“茶棕”主题只提供同目录内置的 Ma Shan Zheng、Long Cang、Liu Jian Mao Cao 三款毛笔字体并默认 Ma Shan Zheng。图片标签的字号和底牌透明度可调，其他主题和固定样式控件按 `viewer.js` 的主题合同执行。
