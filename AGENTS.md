...
2026-09-25 核对：正式 8765 `/api/health` 返回 `owner=true`、`data_dir` 为项目 `data/`、`runtime_version=252516f`（2026-09-21 提交，已包含地点层级与地图稳定聚合）；最近一次扫描为 2026-09-25 对 `I:\shiguang-temp-inbox` 的增量任务，已结束（`completed_with_errors`，1 张截断图读取失败）。人名底牌单图伸缩已提交为 165aad3；运行进程未加载其后对 `app.py`、`home_recommendations.py` 等的提交。前端静态文件由正式服务直接从磁盘提供，后端仍以运行进程版本为准。不要为了让正式服务加载新改动而自行重启；须先备份并确认扫描安全窗口。
...
本机 `dist/拾光相册-绿色版/` 是 2026-09-15 生成的本地草稿（清单 `source_dirty=true`，来源 `4d27c31`），2026-09-25 复核仍未重建、未完成绿色包验收；不把它当作发布物或当前正式运行态。下一步仍是人工核对人物、在安全窗口决定是否让正式后端加载 HEAD，以及完成绿色包验收。
