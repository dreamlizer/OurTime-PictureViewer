...
def main():
    ...
    try:
        ...
        with sync_playwright() as playwright:
            ...
            check(page.locator('#quick-merge-panel').is_visible() and page.locator('#quick-merge-title').inner_text() == '合并到已有姓名', '快速命名合并区默认展开并使用独立标题', results)
            ...
            page.locator(f'[data-quick-name="{quick_section_source}"]').click()
            page.wait_for_selector('#quick-name-dialog[open]')
            check(page.locator('#quick-merge-panel').is_visible() and page.locator('#quick-merge-search').input_value() == '', '快速命名重新打开后合并区保持展开且草稿已重置', results)
            page.fill('#quick-merge-search', '重名测试')
            page.wait_for_function(f"Array.from(document.querySelectorAll('#quick-merge-target option')).some(o => o.value === '{duplicate_target}')")
