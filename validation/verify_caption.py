...
with sync_playwright() as pw:
    ...
    a=open_photo(page)
    check(page.locator('#signature-format').inner_text()=='JPEG · 601.5 KB','底部显示原文件格式和大小，移除分辨率和孤立时间标签')
    check('拍摄时间' in page.locator('#signature-date').inner_text() and '2014.04.22' in page.locator('#signature-date').inner_text(),'时间标签和具体日期在同一区域相邻显示')
    ...
    page.click('#zoom-fit');page.click('#signature-format');closed(page)
    check(True,'点击照片底部白边可关闭大图')
    open_photo(page);page.click('#viewer-context');closed(page);check(True,'点击顶部照片外区域可关闭大图')
    ...
    check(page.locator('.signature-time-value').evaluate('(e)=>e.scrollWidth<=e.clientWidth+1'),'窄屏时间值完整显示')
    page.screenshot(path=str(ROOT/'validation/reports/18-caption-narrow.png'))
    page.click('#signature-place');closed(page)
    check(not errors,'正式浏览器无 JavaScript 错误')
    browser.close()
end=req('/api/status')
check(start['capabilities']['pid']==end['capabilities']['pid'] and end['job']['status']=='running','界面更新未重启或中断扫描后台')
