...
try:
    ...
    with sync_playwright() as pw:
        ...
        page.goto(URL);page.wait_for_selector('#home-query-host .ot-home-ui');page.wait_for_timeout(500)
        page.screenshot(path=str(REPORTS/'01-empty-library.png'),full_page=True)
        page.click('#add-folder');page.evaluate('(path)=>window.__ourTimeApp.openFolder(path)',str(PHOTOS));page.wait_for_selector('#folder-dialog[open]');page.click('#use-folder');page.click('#start-scan')
        ...
        page.click('[data-view="timeline"]');page.wait_for_selector('.photo-card');page.wait_for_function("document.querySelector('#s-assets').textContent==='8'");page.screenshot(path=str(REPORTS/'02-populated-library.png'),full_page=True,animations='disabled')
        ...
        page.set_viewport_size({'width':760,'height':900});page.click('[data-view="timeline"]');page.wait_for_selector('.photo-card')
